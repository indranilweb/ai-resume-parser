...
const ResumeDetailsModal: React.FC<ResumeDetailsModalProps> = ({ 
  ...
  return (
    ...
    >
      <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-xl p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto modal-scroll">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-gray-100">Resume Details</h2>
          <button 
            onClick={onClose}
            className="text-gray-400 hover:text-gray-100 focus:outline-none p-1 rounded hover:bg-gray-900 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-900 rounded-lg p-6 mb-6">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-2xl font-bold text-white mb-2">{resume.name || 'Unknown'}</h3>
              ...
              <div className="bg-gray-900 rounded-lg p-4">
                ...
                <div className="flex flex-wrap">
                  {resume.top_5_technical_skills && resume.top_5_technical_skills.length > 0 ? (
                    ...
                    <span className="text-gray-400 text-sm">No skills information available</span>
                  )}
                </div>
              </div>
            </div>
                ...
                <ul className="space-y-2">
                  {validCompanies.length > 0 ? (
                    validCompanies.map((company, index) => (
                      <li key={index} className="text-gray-100 text-sm mb-2 flex items-center">
                        <div className="w-1.5 h-1.5 bg-blue-400 rounded-full mr-3"></div>
                        {company}
                      </li>
                    ))
                  ) : (
                    <li className="text-gray-400 text-sm">No company information available</li>
                  )}
                </ul>
              </div>
            </div>
